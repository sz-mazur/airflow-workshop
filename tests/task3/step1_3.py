...
def test_exist_dag_step_1_3() -> None:
    dag_status = get_response_from_api(config.DAG_URI_1_3, True).status_code
    assert HTTP_STATUS_OK == dag_status


@pytest.mark.airflow_required
def test_settings_dag_step_1_3() -> None:
    dag_response = get_response_from_api(config.DAG_URI_1_3 + "/details", True).json()

    assert config.DAG_NAME_STEP1_3 == dag_response["dag_id"]
    assert config.SCHEDULE == dag_response["schedule_interval"]["value"]
    assert config.TAGS_STEP1 == dag_response["tags"]


@pytest.mark.airflow_required
def test_data_dag_step_1_3() -> None:
    dag_response = get_response_from_api(config.DAG_URI_1_3 + "/details", True).json()
...
@pytest.mark.airflow_required
def test_catchup_dag_step_1_3() -> None:
    dag_response = get_response_from_api(config.DAG_URI_1_3 + "/details", True).json()

    assert config.CATCHUP == dag_response["catchup"]
